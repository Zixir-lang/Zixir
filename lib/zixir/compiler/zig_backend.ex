defmodule Zixir.Compiler.ZigBackend do
  ...
  @compile {:nowarn_unused_function, [:generate_pattern, :generate_match_expression]}
  ...
  defp generate_program({:program, statements}, indent) do
    ...
    has_main = Enum.any?(statements, fn
    ...
    if has_main do
      ...
      # No main: wrap in pub fn main(); preamble already provides std/zixir
      {stmts, last_expr} = split_statements_and_last(statements)
      stmts_zig = Enum.map(stmts, &generate_statement(&1, indent + 2)) |> Enum.join("\n\n")
      last_str = if last_expr do
        expr_str = generate_expression(last_expr, 0)
        """
        const _result = #{expr_str};
        std.debug.print("{d}\\n", .{_result});
        """
      else
        ""
      end
      "\npub fn main() !void {\n" <> stmts_zig <> (if stmts_zig != "", do: "\n\n", else: "") <> last_str <> "\n}\n"
    end
  end

  defp split_statements_and_last(statements) do
    case statements do
      [] -> {[], nil}
      [_] ->
        [last] = statements
        if expression_statement?(last), do: {[], last}, else: {[last], nil}
      _ ->
        [last | rest] = Enum.reverse(statements)
        if expression_statement?(last) do
  ...
  defp expression_statement?(stmt) do
    case stmt do
      {:var, _, _, _} -> true
      {:call, _, _} -> true
      {:binop, _, _, _} -> true
  ...
  defp generate_statement({:let, name, expr, _line, _col}, indent) do
    ...
    # Zig: use const (Zixir let is immutable); omit type when anytype
    if type_str == "anytype" do
      "#{indent_str}const #{name} = #{expr_str};"
    else
      "#{indent_str}const #{name}: #{type_str} = #{expr_str};"
    end
  end
  ...
  defp generate_expression({:call, func, args}, indent) do
    ...
    args_str = args_to_zig_string(func, args, indent)
    "#{func_str}(#{args_str})"
  end

  defp args_to_zig_string({:field, {:var, "engine", _, _}, field}, args, indent) when field in ["list_sum", "list_product"] do
    args
    |> Enum.with_index()
    |> Enum.map(fn {arg, i} ->
      expr_str = generate_expression(arg, indent)
      if i == 0, do: "&#{expr_str}", else: expr_str
    end)
    |> Enum.join(", ")
  end

  defp args_to_zig_string({:field, {:var, "engine", _, _}, "dot_product"}, args, indent) do
    args
    |> Enum.with_index()
    |> Enum.map(fn {arg, i} ->
      expr_str = generate_expression(arg, indent)
      if i in [0, 1], do: "&#{expr_str}", else: expr_str
    end)
    |> Enum.join(", ")
  end

  defp args_to_zig_string(_func, args, indent) do
    args
  ...
  defp generate_expression({:array, elements, _line, _col}, indent) do
    ...
    |> Enum.map(&generate_expression(&1, indent))
    |> Enum.join(", ")
    arr_type = infer_array_type(elements)
    # Zig disallows "anytype" in array type; omit so Zig infers
    if arr_type == "anytype" do
      "[_]{#{elems_str}}"
    else
      "[_]#{arr_type}{#{elems_str}}"
    end
  end
